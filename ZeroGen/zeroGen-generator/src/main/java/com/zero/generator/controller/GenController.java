...
import com.zero.common.core.domain.CxSelect;
...
import com.zero.common.utils.security.PermissionUtils;
...
public class GenController extends BaseController {
    ...
    public String gen(Model model) {
        List<GenDatasourceConf> list = genDatasourceConfService.selectGenDatasourceConfList(null);
    ...
    public TableDataInfo dataList(GenTable genTable) {
        startPage();
        List<GenTable> list = genTableService.selectDbTableList(genTable, genTable.getDbName());
        return getDataTable(list);
    }
    ...
    @ResponseBody
    public AjaxResult importTableSave(String dbName, String tables) {
        ...
        //查询列信息
        List<GenTableColumn> genTableColumns = genTableColumnService.selectDbTableColumnsByNames(tableNames, dbName);

        genTableService.importGenTable(tableList, operName, dbName, genTableColumns);
    ...
    @RequiresPermissions("tool:gen:edit")
    @Log(title = "代码生成", businessType = BusinessType.UPDATE)
    @GetMapping("/synchDb/{dbName}/{tableName}")
    @ResponseBody
    public AjaxResult synchDb(@PathVariable("tableName") String tableName, @PathVariable("dbName") String dbName) {
        //1.查本地库信息
        GenTable table = genTableService.selectGenTableByName(tableName);

        //2.查目标库信息
        List<GenTableColumn> dbTableColumns = genTableColumnService.selectDbTableColumnsByName(tableName, dbName);

        //3.同步本地库信息
        genTableService.synchDb(table, dbTableColumns);
