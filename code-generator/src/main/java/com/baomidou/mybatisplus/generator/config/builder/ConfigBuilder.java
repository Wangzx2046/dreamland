...
public class ConfigBuilder {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        packageInfo.put(ConstVal.ENTITY, joinPackage(config.getParent(), config.getEntity()+ StringPool.DOT +config.getModuleName()));
        packageInfo.put(ConstVal.MAPPER, joinPackage(config.getParent(), config.getMapper()+ StringPool.DOT +config.getModuleName()));
        packageInfo.put(ConstVal.XML, joinPackage(config.getParent(), config.getXml()+ StringPool.DOT +config.getModuleName()));
        packageInfo.put(ConstVal.SERVICE, joinPackage(config.getParent(), config.getService()+ StringPool.DOT +config.getModuleName()));
        packageInfo.put(ConstVal.SERVICE_IMPL, joinPackage(config.getParent(), config.getServiceImpl()+ StringPool.DOT +config.getModuleName()));
        packageInfo.put(ConstVal.CONTROLLER, joinPackage(config.getParent(), config.getController()+ StringPool.DOT +config.getModuleName()));
        ...
        try {
            ...
            try (
                ...
                while (results.next()) {
                    ...
                    //填充补充信息：是否允许为空，字段最大长度
                    field.setAllowNull("YES".equals(field.getCustomMap().get("NULL")));
                    if (field.getType().indexOf("(") != -1) {
                        field.setMaxLength(field.getType().substring(field.getType().indexOf("(") + 1, field.getType().length() - 1));
                    }
                    fieldList.add(field);
                }
            }
